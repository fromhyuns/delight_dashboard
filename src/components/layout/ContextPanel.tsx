    ...
    <div className="flex h-full w-60 shrink-0 flex-col border-r border-black/20 bg-[#1c1c24] text-stone-300">
      ...
      <div className="shrink-0 border-b border-white/[0.07] p-3">
        <div className="mb-2.5 flex items-center justify-between">
          ...
          <button className="flex items-center gap-1 rounded px-1.5 py-1 text-[10px] text-stone-500 transition hover:bg-white/10 hover:text-stone-200">
            <Plus size={11} />
            New
          </button>
        </div>
        <div className="flex items-center gap-2 rounded-md border border-white/10 bg-white/[0.05] px-2.5 py-1.5">
          <Search size={11} className="shrink-0 text-stone-600" />
          <input
            value={query}
            onChange={(e) => setQuery(e.target.value)}
            placeholder="Search agents…"
            className="min-w-0 flex-1 bg-transparent text-xs text-stone-300 placeholder-stone-600 outline-none"
          />
          {query && (
      ...
      <div className="compact-scrollbar flex-1 overflow-y-auto py-1">
        ...
        {filtered.length === 0 && (
          <p className="px-4 py-3 text-xs text-stone-600">No agents found</p>
        )}
      </div>
    </div>
  );
  ...
  return (
    <div className="flex h-full w-60 shrink-0 flex-col border-r border-black/20 bg-[#1c1c24] text-stone-300">
      ...
      <div className="compact-scrollbar flex-1 overflow-y-auto py-1">
        ...
        {filtered.length === 0 && (
          <p className="px-4 py-3 text-xs text-stone-600">No workspaces found</p>
        )}
      </div>
    </div>
  );
